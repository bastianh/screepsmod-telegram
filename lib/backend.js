  ...
  bot.command('status', async (ctx) => {
    ...
    try {
      ...
      const lines = users.map(u => {
        const checks = []
        if (u.email) {
          checks.push('✓ Email set — notifications will be delivered')
        } else {
          checks.push('✗ No email set — notifications will NOT be delivered\n  Fix: set any email address in your Screeps account settings')
        }
        if (u.notifyPrefs && u.notifyPrefs.disabled) {
          checks.push('✗ Notifications disabled in account settings')
        }
